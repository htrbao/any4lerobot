...
_BASE_FEATURES = {
    ...
    # amodal "trace": agentview_rgb with occluded exo-landmark regions
    # inpainted from their last-visible pixels (see `_build_trace_video`).
    "observation.images.agentview_trace_rgb": {
        "dtype": "video",
        "shape": (256, 256, 3),
        "names": ["height", "width", "rgb"],
    },
...
def _gaussian_patch_mask(radius: int) -> np.ndarray:
    """Soft circular alpha mask with Gaussian falloff, shape (2r+1, 2r+1) float32."""
...
def _build_trace_video(
    rgb_video: np.ndarray,     # (T, H, W, 3) uint8
    positions: np.ndarray,     # (T, max_landmarks, 2) float32 — (cx, cy) per slot/frame
    is_occluded: np.ndarray,   # (T, max_landmarks) bool
    source_frame: np.ndarray,  # (T, max_landmarks) int64 — frame `positions` was sampled from
    patch_radius: int = 18,
) -> np.ndarray:
    """Amodal "trace" video: each occluded landmark's screen region is
    replaced with the pixel patch from the frame it was last visible in,
    blended 50/50 with the current frame via a soft Gaussian mask to avoid
    hard seams. Mirrors co-tracker's demo_amodal.py build_inpainted_video,
    but sources track points from metainfo.json bbox centers instead of a
    CoTracker model.
    """
    T, H, W, _ = rgb_video.shape
    n_slots = is_occluded.shape[1]
    kernel = _gaussian_patch_mask(patch_radius)
    out = np.empty_like(rgb_video)

    for t in range(T):
        frame = rgb_video[t].astype(np.float32)
        weight_acc = np.zeros((H, W), dtype=np.float32)
        color_acc = np.zeros((H, W, 3), dtype=np.float32)

        for n in range(n_slots):
            if not is_occluded[t, n]:
                continue
            tf = int(source_frame[t, n])
            cx = int(round(positions[tf, n, 0]))
            cy = int(round(positions[tf, n, 1]))

            x1 = max(0, cx - patch_radius); x2 = min(W, cx + patch_radius + 1)
            y1 = max(0, cy - patch_radius); y2 = min(H, cy + patch_radius + 1)
            kx1 = x1 - (cx - patch_radius); kx2 = kx1 + (x2 - x1)
            ky1 = y1 - (cy - patch_radius); ky2 = ky1 + (y2 - y1)

            w = kernel[ky1:ky2, kx1:kx2]
            src_patch = rgb_video[tf, y1:y2, x1:x2].astype(np.float32)
            weight_acc[y1:y2, x1:x2] += w
            color_acc[y1:y2, x1:x2] += w[..., None] * src_patch

        mask = weight_acc > 0
        if mask.any():
            historical = color_acc[mask] / weight_acc[mask, None]
            frame[mask] = 0.5 * (historical + frame[mask])
                ...
                dataset.add_frame(
                    {
                        ...
                        "observation.images.agentview_trace_rgb": agentview_trace_rgb[i],
